...
Here's a fact you might not know: Katalon Studio's GUI gives the illusion that a test case is a single object containing Groovy scripts and metadata like “descriptions,” “tags,” and “variables.” However, on the filesystem, test cases are implemented as a combination of two folders (`Test Cases` and `Scripts`) and the files they contain. I inspected the file/folder tree of the project using the link:https://linux.die.net/man/1/tree[tree] command:
...
[quote]
____
What is `1761087607376` in the script file name? It is a timestamp representing the moment when the script file was created. Katalon Studio uses this timestamp to uniquely identify each script file.
____
